...
FinalOrder=9; %(2^FinalOrder)+1 number of nodes used along one side
RunMode = 3;    %1 for normal operation, 2 to examine error, 3 for execution testing
...
SolveType = 1;  %1 for inverse solution method, %2 for iterative technique
    MaxIter=1150; %Max iterations for iterative technique               
...
if MapType==1
    ...
    elseif SparseType==2
        ...
        clear DiagValue; %No longer needed
    end
elseif MapType==2
    A = spalloc(NNodes,NNodes, NNodes*10);	% allocate the matrix
    for j = 2:(Neta - 1)
