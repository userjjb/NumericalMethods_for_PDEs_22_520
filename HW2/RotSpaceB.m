...
FinalOrder=5; %(2^FinalOrder)+1 number of nodes used along one side
...
if MapType==1
    ...
    elseif SparseType==2
        ...
        %Set-up BCs
        DiagValue(1:Nxi,5) = Bottom;
        DiagValue(NNodes-Nxi+1:NNodes,5) = Top;
        DiagValue(Nxi+1:Nxi:Neta*Nxi+1,5) = Left;
        DiagValue(2*Nxi:Nxi:(Neta+1)*Nxi,5) = Right;
...
%=========================================================
Solution = A\F';
MaxDeflect((t/2)+1,(p/2)+1) = max(max(Solution));
